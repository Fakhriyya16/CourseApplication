    ...
    public class TeacherController
    {
        ...
        Id: ConsoleColor.Cyan.ConsoleMessage("Add your id: ");
            ...
            try
            {
                if (!isCorrectFormat)
                {
                    ConsoleColor.Red.ConsoleMessage(string.Format(ResponseMessages.WrongFormat, "id") + "Add Id again");
                    goto Id;
                }
                else
                {
        ...
        public void GetAll()
        {
            ...
            foreach (var teacher in teachers)
            {
                Console.WriteLine(string.Format(ResponseMessages.TeacherDataForDisplay, teacher.Id, teacher.Name, teacher.Surname, teacher.Email));
            }
        }
        public void Login()
        {
        ...
        Id: ConsoleColor.Cyan.ConsoleMessage("Enter ID of the student you want to grade: ");
            ...
            {
            Grade: ConsoleColor.Cyan.ConsoleMessage("Add grade (0-100):");
                string gradeStr = Console.ReadLine();
                int gradeId;
                bool isCorrectFormatOfGrade = int.TryParse(gradeStr, out gradeId);
                if (!isCorrectFormatOfGrade)
                {
                    ConsoleColor.Red.ConsoleMessage(ResponseMessages.WrongFormat);
                    goto Grade;
                }
                try
                {
                    _teacherService.GradeStudent(studentId, gradeId);
                    ConsoleColor.Green.ConsoleMessage("Grade successfully added");
                }
                catch (DataNotFoundException ex)
                {
                    ConsoleColor.Red.ConsoleMessage(ex.Message);
